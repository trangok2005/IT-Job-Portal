"""
Admin chạy lần đầu khi khởi tạo taxonomy, có thể chạy lại an toàn:
...
class Command(BaseCommand):
    help = "Seed taxonomy skill từ skill_taxonomy_seed.json (idempotent)."

    def handle(self, *args, **options):
        seed_path = settings.BASE_DIR / "skill_taxonomy_seed.json"
        if not seed_path.exists():
            self.stderr.write(self.style.ERROR(f"Không tìm thấy {seed_path}"))
            return
        ...
        self.stdout.write(self.style.SUCCESS(
            ...
            f"Tổng: {SkillCategory.objects.count()} category, "
